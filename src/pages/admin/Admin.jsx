    ...
    <>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          {error && <div className="empty">Error: {error}</div>}
          {!error && !data && <div className="empty">Loading\u2026</div>}
          {!error && data && !hasRows && <div className="empty">Δεν βρέθηκαν {tabLabel}.</div>}
          {!error && data && hasRows && (
            <table>
              <thead><tr>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          ...
          {!error && data?.data.length > 0 && (
            <table>
              <thead><tr>
                <th>Ημερομηνία</th><th>Πελάτης</th><th>Email</th><th>Τύπος</th><th>Αριθμός</th><th style={{ textAlign: 'right' }}>Ποσό</th><th>Νόμ.</th><th>Πηγή</th><th>Πληρωμή</th><th>Παραστατικό</th><th>Λεπτομέρειες</th><th></th>
              </tr></thead>
              <tbody>
                {data.data.map((p, idx) => {
  ...
  return (
    <>
      <div className="card" style={{ marginTop: 24 }}>
        <div className="card-header"><h2>Stripe Τιμές</h2><small style={{ color: 'var(--gray-400)' }}>Stripe Price IDs για κάθε υπηρεσία — ίδια τιμή για B2C & B2B (η διαφοροποίηση γίνεται μέσω ξεχωριστών Payment Links)</small></div>
        <div className="card-body" style={{ padding: 0 }}>
